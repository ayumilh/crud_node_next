...
const FormEditarFuncionarios = () => {
  ...
  const func_codigo = Cookies.get('selectedFuncionario');
    ...
    const fetchFuncionario = async () => {
      try {
        const response = await axios.get(
          `https://pos-backend-six.vercel.app/api/funcionarios/getId/${func_codigo}`
        );
        setInputs(response.data.funcionario);
      }catch (error) {
        console.log(error);
      }
    };
  ...
  const handleEditar = async () => {
    const { func_data_atualizacao, func_data_cadastro, func_data_excluido, func_excluido, func_data_nascimento, ...FuncionariosemDatas } = input;
    ...
    if (dataNascimentoFormatada) {
      FuncionariosemDatas.func_data_nascimento = dataNascimentoFormatada;
    }
  
    try {
      const res = await axios.put('https://pos-backend-six.vercel.app/api/Funcionarios/editar', {
        ...FuncionariosemDatas,
        func_codigo
      });
      if (res.status === 200) {
    ...
    <div className="lg:flex lg:gap-28 mx-2 md:mx-4 lg:mx-6">
      <form className="w-full lg:max-w-6xl xl:max-w-7xl">
        <div className="flex flex-col gap-4">
          ...
          {statusRequest === true && (
            <SuccessNotification message="Funcionario editado com sucesso!" />
          )}
          {statusRequest === false && (
            <ErrorNotification message="Não foi possível editar o funcionario!" />
          )}
        </div>
      </form>
    </div>
...
export default FormEditarFuncionarios;